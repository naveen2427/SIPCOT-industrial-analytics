...
import { Search, Building, MapPin, CheckCircle, Clock, XCircle, Filter, Layers } from 'lucide-react';
...
const Companies = () => {
  ...
  useEffect(() => {
    ...
    if (sectorParam) {
      setFilterSector(sectorParam);
    }
    
    // Load dynamically added companies from local storage
    const submissions = JSON.parse(localStorage.getItem('mockSubmissions') || '[]');
    const newCompanies = submissions
      ...
      .map(s => ({
        ...
        park: s.park,
        status: s.status,
        score: s.status === 'Approved' ? Math.floor(Math.random() * 20) + 70 : 0,
        investment: 'Under Review'
      }));
      
    setAllCompanies([...mockCompanies, ...newCompanies]);
  }, [searchParams]);
