...
const CertificatePage = () => {
  ...
  React.useEffect(() => {
    ...
    if (id && id !== 'mock-id-new') {
      found = submissions.find(s => s.id.toString() === id);
    } 
    
    if (!found && user?.role === 'company_admin') {
      found = [...submissions].reverse().find(s => s.company === user.name && s.status === 'Approved');
    }

    if (found) {
      setCertData({
        certId: `CERT-2025-${found.id}`,
