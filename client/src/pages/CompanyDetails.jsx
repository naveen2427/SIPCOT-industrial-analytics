...
const CompanyDetails = () => {
  ...
  React.useEffect(() => {
    if (parseInt(id) > 9) {
      ...
      const found = submissions.find(s => s.id.toString() === id);
      if (found) {
        setCompany({
          id: found.id,
          name: found.company,
          ...
          park: found.park,
          status: found.status,
          certId: found.status === 'Approved' ? `CERT-2025-${found.id}` : 'Pending',
          investment: 'Data Pending',
          employees: 'Data Pending',
          revenue: 'Data Pending',
          score: 0,
          description: `Newly registered company in the ${found.sector} sector located in ${found.park} SIPCOT Industrial Park.`,
          isNew: true
        });
      }
    }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-8">
                ...
                {company.isNew ? (
                  <div>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-12 text-center text-slate-500 flex flex-col items-center justify-center">
                       <TrendingUp className="text-slate-300 mb-4" size={48} />
                       <h4 className="text-lg font-semibold text-slate-700">No Records Available</h4>
                       <p className="mt-2 text-sm text-slate-500 max-w-sm mx-auto">This company was registered less than 1 year ago. Growth analytics and performance data will be available after the first annual review.</p>
                    </div>
                  </div>
                ) : (
                  <div>
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 rounded-xl p-6 border border-slate-100 shadow-sm text-center">
                  ...
                  {company.isNew ? (
                    <div className="py-8">
                      <div className="text-sm font-bold text-slate-400 bg-slate-200 inline-block px-6 py-2 rounded-full">N/A</div>
                    </div>
                  ) : (
                  ...
                  <p className="text-xs text-slate-500 mt-4">{company.isNew ? 'Score generated after 1 year' : 'Based on 3-year performance metrics'}</p>
                </div>
